...
const AddToPlanForm = ({ plan, open, onClose }) => {
  ...
  useEffect(() => {
    if (plan) {
      setFormData({ ...plan, vulnerabilites: plan.vulnerabilites || [] });
    }
  }, [plan]);

  const handleChange = (e) => setFormData({ ...formData, [e.target.name]: e.target.value });
  const handleSelectChange = (field, value) => setFormData({ ...formData, [field]: value });
  const handleCommentChange = (field, value) => setFormData({ ...formData, [field]: value });
  ...
  const handleSubmit = async () => {
    try {
      ...
      await api.post(`/plan/plan`, dataToSend);
      toast.success("Audit ajouté au plan avec succès !");
      onClose();
    } catch (error) {
  ...
  const handleVulnChange = (index, field, value) => {
    const newVulns = formData.vulnerabilites.map((v, i) =>
