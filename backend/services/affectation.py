...
def generate_affect_pdf(affect):
    ...
    contacts = affect.demande_audit.contacts if hasattr(affect.demande_audit, "contacts") else []

    # Render the HTML with the data
    html_content = template.render(affect=affect, logo_path=logo_path_url, contacts=contacts)
...
def create_affect(db: Session, affect_data: AffectSchema):
    ...
    for ip_data in affect_data.ips:
        existing_ip = db.query(IP).filter(IP.adresse_ip == ip_data.adresse_ip).first()

        if existing_ip:
            if existing_ip.status == "open":
                logger.warning(f"L'IP {ip_data.adresse_ip} existe déjà avec le statut 'open' => Ignorée")
                continue  # Ne pas enregistrer une IP déjà présente avec statut "open"
            else:
                logger.info(f"Mise à jour du statut de l'IP {ip_data.adresse_ip} à 'open'")
                existing_ip.status = "open"
                db.commit()
                affect.ips.append(existing_ip)
                continue

        # IP inexistante => on la crée
